...
export async function POST(request: NextRequest) {
    try {
        ...
        const counts = await gsc.getUniqueCounts(siteUrl, startDate, endDate, country);

        return NextResponse.json({ success: true, data: counts });
    } catch (error: any) {
        console.error('GSC unique counts error:', error);
        return NextResponse.json(
            { error: error.message || 'Failed to get unique counts' },
            { status: 500 }
        );
    }
}
